          ...
          <div className="flex flex-col lg:flex-row shadow-lg rounded-lg overflow-hidden bg-white">
            ...
            <div className="lg:w-3/4 p-6 border-r border-gray-200">
              <div className="flex justify-between border-b border-gray-200 pb-4 mb-4">
                ...
                <h2 className="text-xl font-semibold text-gray-600">{cart.length} Items</h2>
              </div>
              {cart.length === 0 ? (
                ...
                cart.map((item) => (
                  <div className="flex items-center border-b border-gray-200 py-4" key={item.id}>
                    <div className="flex w-2/5">
                      <img className="w-24 h-24 object-cover rounded-lg" src={item.thumbnail} alt={item.title} />
                    ...
                    <span className="w-1/5 text-center text-gray-800">${item.price.toFixed(2)}</span>
                    <span className="w-1/5 text-center text-gray-800">${(item.quantity * item.price).toFixed(2)}</span>
                  </div>
                ))
              )}
              <Link to="/" className="flex items-center text-indigo-600 text-sm mt-6 hover:underline">
                <svg className="w-4 h-4 mr-2" viewBox="0 0 448 512" fill="currentColor">
            ...
            <div className="lg:w-1/4 p-6 bg-gray-50">
              <h1 className="text-2xl font-semibold border-b border-gray-200 pb-4">Order Summary</h1>
              <div className="flex justify-between mt-6 mb-4">
                <span className="font-semibold text-gray-800">Items {cart.length}</span>
              ...
              <div className="mb-6">
                ...
                <input type="text" className="block w-full p-2 border border-gray-300 rounded-md text-gray-600" placeholder="Enter your code" />
              </div>
              <button className="w-full bg-red-500 text-white py-2 rounded-md hover:bg-red-600">Apply</button>
              <div className="border-t border-gray-200 mt-6 pt-6">
                <div className="flex justify-between text-gray-800 font-semibold text-sm">
                  <span>Total Cost</span>
                  <span>${(total + 10).toFixed(2)}</span>
                </div>
                <button className="w-full mt-4 bg-indigo-500 text-white py-2 rounded-md hover:bg-indigo-600">Checkout</button>
              </div>
            </div>
          </div>
        </div>
